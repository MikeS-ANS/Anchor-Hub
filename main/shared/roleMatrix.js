...
// Returns all Azure AD App Roles assigned to the signed-in user.
// A user can have multiple roles (e.g. hub.manager + hub.finance for a one-off grant).
...
// Convenience: returns the primary role (first one) for display purposes.
async function getUserRole() {
  const roles = await getUserRoles();
  return roles[0] || null;
}

// Returns the Set of tool keys the user can access — union across all their roles.
// Returns null if the matrix isn't set up yet (caller shows all tools as fallback).
